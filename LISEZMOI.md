...
| YouTube / Vimeo / SoundCloud / Twitch | correspondance d'hôte | détecté mais **pas encore implémenté** : `PodcastHelperError::YtDlpNotImplemented` (voir ci-dessous) |
...
- `cargo test` : **32 tests unitaires + 1 doctest passés**, 0 échec, 3 tests d'intégration réseau marqués `#[ignore]` (flux RSS réel, décodage `ffmpeg` bout-en-bout, refus Spotify) — vérifiés manuellement une fois via `cargo test -- --ignored` (les 3 passent) avant de committer, pas exécutés par défaut.
